...
function withRoomRequestTimeout<T>(request: Promise<T>) {
  return Promise.race<T>([
    request,
    new Promise<T>((_, reject) => {
      window.setTimeout(
        () => reject(new Error("The room request timed out.")),
...
export default function CallStudio() {
  ...
  const joinCall = async () => {
    ...
    try {
      ...
      const roomSnapshot = await withRoomRequestTimeout(getDoc(currentRoomRef));
      ...
      setStatus(`We couldn't join that call: ${detail}`);
      setCallRole("idle");
    } finally {
      setIsBusy(false);
    }
            ...
            <div className="space-y-5">
              ...
              {!user ? (
                <div className="rounded-3xl border border-sky-100 bg-sky-50/70 p-5">
                  ...
                  <Button className="mt-4 bg-red-600 hover:bg-red-700 text-white" onClick={requestSignIn}>
                ...
                <div className="grid gap-4 rounded-3xl border border-slate-200 bg-slate-50/80 p-5 md:grid-cols-2">
                  <div className="space-y-3 rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
                    <p className="text-sm font-medium text-slate-700">Start a room</p>
                    ...
                    <Button 
                      ...
                      className="mt-2 bg-red-600 hover:bg-red-700 text-white font-semibold shadow-md active:scale-95 transition-all"
                    >
                      <PhoneCall className="h-4 w-4" />
                  ...
                  <div className="space-y-3 rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
                    <p className="text-sm font-medium text-slate-700">Join a friend</p>
                    <Input
                      value={roomInput}
                      onChange={(event) => setRoomInput(event.target.value)}
                      placeholder="Paste room code"
                      disabled={isBusy || inCall}
                      className="w-full rounded-xl border border-slate-300 bg-slate-100 px-3 py-2 text-slate-900 placeholder:text-slate-500 focus:border-red-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-red-500"
                    />
                    <Button
                      variant="outline"
                      onClick={joinCall}
                      disabled={isBusy || inCall}
                      className="w-full border-slate-300 font-medium hover:bg-slate-100 text-slate-900"
                    >
