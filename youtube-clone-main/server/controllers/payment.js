...
export const createPlanOrder = async (req, res) => {
  ...
  const planCode = String(req.body.planCode || req.body.plan || "gold").toLowerCase();
...
export const verifyPlanPayment = async (req, res) => {
  ...
  try {
    ...
    await payments.findByIdAndUpdate(updatedPayment._id, {
      invoiceEmailSent: emailResult.delivered,
      invoiceEmailSentAt: emailResult.delivered ? new Date() : undefined,
    });
